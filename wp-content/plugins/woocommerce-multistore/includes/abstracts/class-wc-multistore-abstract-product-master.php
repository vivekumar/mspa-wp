...
//		'_wp_old_slug',
	...
	public function delete_sync_data($site_id) {
		...
		$woonet_settings = $this->wc_product->get_meta('_woonet_settings');
		...
		if( is_array( $woonet_settings ) ){
			$publish_to = '_woonet_publish_to_' . $site_id;
			$inherit = '_woonet_publish_to_' . $site_id . '_child_inheir';
			$stock = '_woonet_' . $site_id . '_child_stock_synchronize';
			unset($woonet_settings[$publish_to]);
			unset($woonet_settings[$inherit]);
			unset($woonet_settings[$stock]);
			update_post_meta($this->wc_product->get_id(), '_woonet_settings', $woonet_settings );
		}
	}
