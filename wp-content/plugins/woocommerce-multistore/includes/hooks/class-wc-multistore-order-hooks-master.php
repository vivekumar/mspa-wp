...
class WC_Multistore_Order_Hooks_Master {
	...
	public function hooks(){
		...
		// hpos
		add_filter( 'woocommerce_shop_order_list_table_columns', array( $this, 'set_custom_edit_post_columns' ), 99 );
		add_action( 'woocommerce_shop_order_list_table_custom_column', array( $this, 'print_hpos_order_originating_column' ), 10, 2 );
	...
	public function print_hpos_order_originating_column( $column, $order ){
		if( WOO_MULTISTORE()->settings['enable-order-import'] != 'yes' ){ return; }
		switch ( $column ) {
			case 'woonet-order-originating':
				$text = $order->get_meta( 'WOONET_PARENT_ORDER_ORIGIN_TEXT', true, 'edit' );
				$parent_id = $order->get_meta( 'WOONET_PARENT_ORDER_ORIGIN_ID', true, 'edit' );
				$url = $order->get_meta( 'WOONET_PARENT_ORDER_ORIGIN_URL', true, 'edit' ) . '/wp-admin/post.php?post='.$parent_id.'&action=edit';
				echo "<a target='_blank' href='" . $url . "'>" . $text . "</a>";
				break;
		}
	}
