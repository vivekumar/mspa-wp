	...
	public function enqueue_assets() {
		if ( is_admin() ) {
			if( isset($_GET['page']) && $_GET['page'] == 'woonet-bulk-update-products' ){
				...
				wp_register_style( 'wc-multistore-bulk-update-css', WOO_MSTORE_ASSET_URL .'/assets/css/wc-multistore-bulk-update.css', array('select2-css'), '1.0.1' );
				wp_enqueue_style( 'wc-multistore-bulk-update-css' );

				wp_register_script( 'wc-multistore-bulk-update-js', WOO_MSTORE_ASSET_URL . '/assets/js/wc-multistore-bulk-update.js', array('select2'), '1.0.1' );
		...
		if ( ! empty( $_POST['data'] ) ) {
			...
			if ( is_array( $params ) && count( $params ) >= 1 ) {
				...
				if ( ! empty( $params['select-all-products'] ) ) {
					...
					// categories
					if( ! empty( $params['select_categories'] ) ){
					...
					if( ! empty( $attribute_taxonomies ) ){
						foreach ( $attribute_taxonomies as $attribute_taxonomy ){
							$attribute_input = "bulk_update_pa_" . $attribute_taxonomy->attribute_name;
							if( ! empty( $params[$attribute_input] ) ){
								$args['tax_query'][] = array(
									'taxonomy' => 'pa_'.$attribute_taxonomy->attribute_name,
									'field'    => 'term_id',
									'terms'    => $params[$attribute_input],
								);
							}
						}
					}
