...
class WC_Multistore_Network_Orders {
	...
	public function add_network_submenu_page() {
		add_submenu_page( 'woonet-woocommerce', __( 'Network Orders', 'woonet' ), __( 'Network Orders', 'woonet' ), 'manage_woocommerce', 'network-orders', array( $this, 'orders_interface' ), 4 );
	}
	...
	public function add_submenu_page() {
        if( ! is_multisite() ){
	        add_submenu_page('woonet-woocommerce','Network Orders','Network Orders','manage_woocommerce','network-orders', array( $this, 'orders_interface' ) );
        }
	}
                            ...
                            <?php foreach ( $network_orders['orders'] as $order_data ) : ?>
				                <?php if ( ! empty( $order_data ) ) : ?>
                                    <tr class="post-<?php echo esc_attr( $order_data['store_name'] ); ?>_<?php echo esc_attr( $order_data['id'] ); ?> type-shop_order status-<?php echo esc_attr( $order_data['status'] ); ?> post-password-required hentry" id="post-<?php echo esc_attr( $order_data['site_id'] ); ?>_<?php echo esc_attr( $order_data['id'] ); ?>">
                                        ...
                                        <td class="billing_address column-billing_address"><?php $this->render_shop_order_columns( 'billing_address', $order_data, $order_data['site_id'] ); ?></td>
                                        <td class="shipping_address column-shipping_address"><?php $this->render_shop_order_columns( 'shipping_address', $order_data, $order_data['site_id'] ); ?></td>
                                        ...
                                        <?php if ( WOO_MULTISTORE()->settings['enable-order-import'] == 'yes' ): ?>
                                            <td class="order_originated column-order_originated"><?php $this->render_shop_order_columns( 'order_originated', $order_data, $order_data['site_id'] ); ?></td>
                                        <?php endif; ?>
                                    </tr>
		                        <?php endif; ?>
                            <?php endforeach; ?>
